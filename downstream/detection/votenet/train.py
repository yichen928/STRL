...
parser.add_argument('--init_weight', type=str, default=None, help='initial weight for training')
parser.add_argument('--freeze_pretrain', action='store_true', default=False, help='whether to freeze loaded pretrained weights')
parser.add_argument('--backbone_finetune_lr', type=float, default=None, help='decline lr for backbone by a ratio')
...
FLAGS.log_dir = os.path.join("logs", FLAGS.log_dir)
...
if INIT_WEIGHT and os.path.isfile(INIT_WEIGHT):
    ...
    for name, param in net.named_parameters():
        if name in checkpoint['model_state_dict']:
            print(name)
            if FLAGS.freeze_pretrain:
                param.requires_grad = False
                print("freeze:", name)
        if FLAGS.backbone_finetune_lr:
            if name in checkpoint['model_state_dict']:
...
if FLAGS.backbone_finetune_lr:
    optimizer = optim.Adam([{"params": others, "lr": BASE_LEARNING_RATE},
                            {"params": backbones, "lr": BASE_LEARNING_RATE * FLAGS.backbone_finetune_lr}],
                           lr=BASE_LEARNING_RATE, weight_decay=FLAGS.weight_decay)
else:
    optimizer = optim.Adam(filter(lambda p: p.requires_grad, net.parameters()), lr=BASE_LEARNING_RATE, weight_decay=FLAGS.weight_decay)
