...
def train(args, io):
    ...
    for epoch in range(args.epochs):
        ...
        train_pred_seg = []
        train_label_seg = []
        for id, (data, seg) in enumerate(train_loader):
            if id % 10 == 0:
                print("iter: %d/%d"%(id+1, len(train_loader)))
        ...
        test_true_seg = []
        test_pred_seg = []
        for id, (data, seg) in enumerate(test_loader):
            if id % 10 == 0:
                print("Test Iter: %d/%d"%(id+1, len(test_loader)))
            data, seg = data.to(device), seg.to(device)
            data = data.permute(0, 2, 1)
...
if __name__ == "__main__":
    ...
    parser.add_argument('--pretrain_weight', type=str, default='',
                        help='Pretrained initial weight')
    ...
    args.train_area = [area for area in args.train_area.split(",")]
